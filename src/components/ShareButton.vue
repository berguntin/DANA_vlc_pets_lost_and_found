      ...
      <Teleport to="body">
        <transition name="fade">
          <div v-if="isActive" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div
              ref="popupRef"
              class="bg-white text-gray-800 shadow-xl rounded-lg p-6 max-w-sm w-full mx-4 relative"
              role="dialog"
              aria-modal="true"
            >
              <button 
                @click="togglePopup" 
                class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-300 rounded-full p-1"
                aria-label="Cerrar"
              >
                <XIcon class="h-6 w-6" />
              </button>
